...
class AES:
    ...
    def SubBytes(self, State):
        for i in range(4):
            for j in range(4):
                self.State[i][j] = self.SBox[State[i][j]]


    def InvSubBytes(self, State):
        for i in range(4):
            for j in range(4):
                self.State[i][j] = self.InvSBox[State[i][j]]


    def ShiftRows(self, State):
        self.State[0][1], self.State[1][1], self.State[2][1], self.State[3][1] = State[1][1], State[2][1], State[3][1], State[0][1]
        self.State[0][2], self.State[1][2], self.State[2][2], self.State[3][2] = self.State[2][2], State[3][2], State[0][2], State[1][2]
        self.State[0][3], self.State[1][3], self.State[2][3], self.State[3][3] = State[3][3], State[0][3], State[1][3], State[2][3]


    def InvShiftRows(self, State):
        self.State[0][1], self.State[1][1], self.State[2][1], self.State[3][1] = State[3][1], State[0][1], State[1][1], State[2][1]
        self.State[0][2], self.State[1][2], self.State[2][2], self.State[3][2] = State[2][2], State[3][2], State[0][2], State[1][2]
        self.State[0][3], self.State[1][3], self.State[2][3], self.State[3][3] = State[1][3], State[2][3], State[3][3], State[0][3]
    ...
    def InvCipher(self, State, Nr, Expanded_KEY):
        ...
        self.AddRoundKey(cipher_state, self.Expanded_KEY[4 * Nr:4 * Nr + 4])   

        for i in range(self.Nr - 1, 0, -1):             
            self.InvShiftRows(cipher_state)
            self.InvSubBytes(cipher_state)
            roundKey = self.Expanded_KEY[4 * i:4 * i + 4]  
            self.AddRoundKey(cipher_state, roundKey)
            self.InvMixColumns(cipher_state)

        self.InvShiftRows(cipher_state)
        self.InvSubBytes(cipher_state)
        self.AddRoundKey(cipher_state, self.Expanded_KEY[0:4])
